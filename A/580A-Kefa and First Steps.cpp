...
3. If current value is not greater than previous value then check if temp_length>max_length and if it is then update max_length.
Also restore value of temp_length.
4. Check once more at the end of the loop if temp_length>max_length and update value of max_length accordingly.
This is done so that if the entire sequence is one increasing subsequence we still update max_length as the else doesn't get triggered in
that scenario so temp_length stores the length of greatest increasing subsequence */
int main() {
